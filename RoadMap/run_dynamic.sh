...
if [ "$1" = "profile" ]; then
  ...
  echo "localhost" > hostfile
  sh generate_hosts.sh $NUM_HOSTS
  mpirun -hostfile hostfile RoadMapGProf_dynamic
  gprof -lb RoadMapGProf_dynamic gmon.*
elif [ "$1" = "gperf" ]; then
  LD_LIBRARY_PATH=/state/partition1/apps/lib:$LD_LIBRARY_PATH CPUPROFILE=profile.out mpirun RoadMapGPerf
    /state/partition1/apps/bin/pprof --lines --text RoadMapGPerf_dynamic profile.out
else
  echo "localhost" > hostfile
  sh generate_hosts.sh $NUM_HOSTS
  mpirun -hostfile hostfile RoadMap_dynamic
fi
